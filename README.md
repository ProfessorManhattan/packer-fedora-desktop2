<!-- ⚠️ This README has been generated from the file(s) "./modules/docs/blueprint-readme.md" ⚠️--><h1 align="center">Packer Template for Fedora Desktop</h1>
...
<p align="center">
  <b>Packer project that builds minimal VagrantUp Fedora Desktop boxes</b></br>
</p>
...
This repository automates most of the process of keeping our Fedora Desktop VM images up-to-date with the latest upstream source by:
...
The example above will build a VirtualBox Fedora Desktop box and upload it to your VagrantUp box repository *(that you have to create before running the script that is above)*. If you want to build the box for another provider then look at the `template.json` in the `"builders"` section for the types of boxes you can create (e.g. virtualbox-iso, vmware-iso, parallels-iso, qemu).
...
If a new release of the Fedora Desktop system is available, you can update your VagrantUp box. You can do this by changing the `"iso_checksum_url"`, `"iso_url"`, and `"iso_version"` variables in the `template.json` file. After that, run `packer build -only=virtualbox-iso template.json` and you should be golden.
...
Contributions, issues, and feature requests are welcome! Feel free to check the [issues page](https://gitlab.com/megabyte-space/packer/Base-Fedora-Desktop/-/issues). If you would like to contribute, please take a look at the [contributing guide](https://gitlab.com/megabyte-space/packer/Base-Fedora-Desktop/-/raw/master/CONTRIBUTING.md).
